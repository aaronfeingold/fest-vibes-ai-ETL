...
4. **Clearing Redis Cache in Development**

   ```bash
   # Connect to Redis CLI
   redis-cli

   # Clear specific date's cache
   DEL "events:2024-03-20"

   # Clear all events (BE CAREFUL with this!)
   redis-cli KEYS "events:*" | xargs redis-cli DEL

   # Clear entire Redis database (BE VERY CAREFUL!)
   FLUSHDB

   # Clear all databases (BE EXTREMELY CAREFUL!)
   FLUSHALL
   ```

   Note: The `FLUSHDB` and `FLUSHALL` commands should be used with extreme caution, especially in production environments.

5. **Debugging Redis Connection Issues**
